...
| 步骤           | 解决的问题    | 说明                          |
| ------------ | -------- | --------------------------- |
| **查询扩展**     | 用户用词不准确  | 生成同义词/变体，提高召回率              |
| **BM25 搜索**  | 快速精确匹配   | 找到含关键词的文档（毫秒级）              |
| **向量搜索**     | 语义相似匹配   | 找到"意思相近"但用词不同的文档            |
| **RRF 融合**   | 合并多路结果   | 去重并综合两种搜索的优势                |
| **Top-K 选择** | 降低计算成本   | 只保留最相关的候选，减少后续计算            |
| **LLM 重排序**  | 精确判断相关性  | 用 Cross-encoder 深度理解查询-文档关系 |
| **位置感知混合**   | 保护高置信度结果 | 防止重排序器过度调整靠前的优质结果           |
...
- [[32_qmd_overview|项目概览]]
- [[33_qmd_store_源码|核心存储模块]]
- [[31_qmd_llm_源码|LLM 集成模块]]
- [[34_qmd_智能分块|智能分块算法]]
- [[36_qmd_设计思想|设计思想总结]]
